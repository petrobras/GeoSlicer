...
1.  **Selecione os Dados:** Na árvore da hierarquia de dados, selecione os itens (volumes, segmentações ou tabelas) que você deseja exportar. Você pode selecionar múltiplos itens.
    * **Dados associados**: Caso um item selecionado possua algum dado associado, a opção **Associated data** ficará disponível e permitirá escolher os tipos de dados para serem exportados juntos. Atualmente apenas os dados de **Proportions** estão disponíveis.
