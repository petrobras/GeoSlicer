...
class TwoPhaseSimulation:
    def __init__(
        ...
        self.params_list = self.get_params_list(self.expand_params(self.params_dict))[slice(*sim_interval)]
    ...
    @staticmethod
    def expand_params(params_dict: dict):
        """
        Expand parameters with start, stop, steps into lists of values.
        If a parameter does not have these keys, keep its value as-is.

        Args:
            params_dict (dict): Dictionary with parameter definitions.

        Returns:
            dict: Dictionary with the same keys, but values expanded to lists.
        """
        expanded = {}
        for param, values in params_dict.items():
            if param == "saturation_steps":
                continue

            if param.startswith("inject_") or param.startswith("produce_"):
                left_val, right_val = values[0], values[1]
                expanded[f"{param}_left"] = "T" if left_val == "1" else "F"
                ...
                elif values.get("step_spacing") == "logarithmic":
                    expanded[param] = np.logspace(np.log10(start), np.log10(stop), steps).tolist()
                else:
                    expanded[param] = np.linspace(start, stop, steps).tolist()
            else:
                expanded[param] = values
        return expanded
