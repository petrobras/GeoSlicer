...
def display_instances(
    ...
    for i in range(N):
        ...
        if not captions:
            ...
            caption = captions[i]
        ax.text(x1, y1 - 5, caption, color="w", size=11, backgroundcolor="none")
...
def display_differences(
    ...
    captions = ["" for m in gt_match] + [
        "{:.2f} / {:.2f}".format(
            pred_score[i], (overlaps[i, int(pred_match[i])] if pred_match[i] > -1 else overlaps[i].max())
        )
...
def draw_boxes(image, boxes=None, refined_boxes=None, masks=None, captions=None, visibilities=None, title="", ax=None):
    ...
    for i in range(N):
        ...
        if boxes is not None:
            ...
            y1, x1, y2, x2 = boxes[i]
            p = patches.Rectangle(
                (x1, y1), x2 - x1, y2 - y1, linewidth=2, alpha=alpha, linestyle=style, edgecolor=color, facecolor="none"
            )
            ax.add_patch(p)
