...
import qt
from qt import QBrush, QColor, QPainter, QPen, QPaintEvent, QFont


class RoundProgressBar(qt.QWidget):
    def __init__(self, parent=None):
        ...
        if self.rpb_dynamicMin:
            self.setMinimumSize(
                qt.QSize(self.lineWidth * 6 + self.pathWidth * 6, self.lineWidth * 6 + self.pathWidth * 6)
            )
    ...
    class lineCapFlags:
        SquareCap = qt.Qt.SquareCap
        RoundCap = qt.Qt.RoundCap
    ...
    def convertInputValue(self, value):
        ...
        if self.rpb_textType == RoundProgressBar.textFlags.Percentage:
            self.rpb_textValue = (
                str(round(((value - self.rpb_minimum) / (self.rpb_maximum - self.rpb_minimum)) * 100)) + "%"
            )
        else:
    ...
    def lineComponent(self):
        ...
        penLine.setJoinStyle(qt.Qt.RoundJoin)
    ...
    def textComponent(self):
        if self.rpb_textActive:
            ...
            textPainter.drawText(
                self.positionX + self.textFactorX, self.positionY + self.textFactorY, self.rpb_textValue
            )
            textPainter.end()
