...
class MicroCTLoaderLogic(LTracePluginLogic):
    ...
    def load(self, path, p, baseName=None):
        if path.suffix == ".nc":
            return self.loadNetCDF(path, p)
        if path.is_file():
            singleFile = True
        ...
        _, images, isVolumes = self.getCountsAndLoadPathsForImageFiles(path)

        nodes = [self.loadImage(file, p, base, singleFile or isVolumes) for file in images]
        return nodes

    def loadNetCDF(self, path, p):
    ...
    def loadImage(self, file, p, baseName, singleFile=False):
        ...
        spacing = [
            ...
            p.imageSpacing2.m_as(SLICER_LENGTH_UNIT),
            p.imageSpacing3.m_as(SLICER_LENGTH_UNIT),
        ]
        ...
        if singleFile:
            node.SetIJKToRASDirections(
                -1 if p.invertDirections[0] else 1,
                0,
                0,
                0,
                -1 if p.invertDirections[1] else 1,
                0,
                0,
                0,
                -1 if p.invertDirections[2] else 1,
            )
    ...
    @staticmethod
    def getImageList(path: Path) -> List[str]:
        paths = []
        for filename in os.listdir(path):
            filepath = path / filename
            if filepath.suffix.lower() in MicroCTLoaderLogic.MICRO_CT_LOADER_FILE_EXTENSIONS:
    ...
    def detectSpacing(path):
        if path.is_file():
            ...
            pathList = [path] + MicroCTLoaderLogic.getImageList(path)
        else:
            return None
        for path in pathList:
            name = Path(path).name

            autoSpacing = MicroCTLoaderLogic.SPACING_REGEX.search(name)
            if not autoSpacing:
                continue
    ...
    def getCountsAndLoadPathsForImageFiles(path: Path) -> Tuple[int, List[Path], bool]:  # -> (count, paths, is3dBatch)
        ...
        try:
            if path.is_file():
                if path.suffix.lower() not in MicroCTLoaderLogic.MICRO_CT_LOADER_FILE_EXTENSIONS:
                    return 0, None, False
                if path.suffix.lower().startswith(".tif"):
                    with TiffFile(path) as tiff:
                        ...
                        return (
                            ...
                            [path],
                            False,
                        )  # always return 1 because the image exist and is read so there is a slice, but sometimes not the attribute num_pages internally
                return 1, [path], False

            elif path.is_dir():
                images = MicroCTLoaderLogic.getImageList(path)
                if not images:
                    return 0, [], False
