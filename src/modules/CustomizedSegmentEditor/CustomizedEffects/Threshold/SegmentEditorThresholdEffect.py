...
<li><b>Apply:</b> set the previewed segmentation in the selected segment. Previous contents of the segment is overwritten.</li>
  ...
  def sourceVolumeNodeChanged(self):
    ...
    self.thresholdInt(sourceVolumeNode)
  ...
  def onUseForPaint(self):
    ...
    # self.scriptedEffect.selectEffect("Paint")  ## avoid switching to Paint effect because it is counter-intuitive
  ...
  def preview(self):
    if self.enablePulsingCheckbox.checkState() == qt.Qt.Checked:
      opacity = 0.1 + self.previewState / (2. * self.previewSteps)
    else:
      opacity = 1.0
    min = self.scriptedEffect.doubleParameter("MinimumThreshold")
    max = self.scriptedEffect.doubleParameter("MaximumThreshold")
  ...
  def thresholdInt(self, node):
    if node is not None:
      narray = slicer.util.arrayFromVolume(node)
      self.isInt = np.issubdtype(narray.dtype, np.integer)

  def updateHistogram(self):
    sourceImageData = self.scriptedEffect.sourceVolumeImageData()
    if sourceImageData is None or self.histogramPipeline is None:
    ...
    scalarType = sourceImageData.GetScalarType()
